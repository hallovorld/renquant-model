...
class TestRawForwardReturnLabel:
    def test_hand_computed_exact_horizon(self) -> None:
        # close[i] = 100 + i on contiguous calendar days:
        # label[D_i] = (100 + i + 20)/(100 + i) - 1, exactly.
        ...
        for i in (0, 17, 39):
            expected = (100.0 + i + 20) / (100.0 + i) - 1.0
            assert label.iloc[i] == pytest.approx(expected)
        # final 20 days have no D+20 bar -> NaN, never a substitute horizon
        assert label.iloc[-20:].isna().all()
        assert label.iloc[:-20].notna().all()

    def test_missing_target_bar_is_nan_not_nearest(self) -> None:
        idx = pd.date_range("2025-01-01", periods=60, freq="D")
        close = pd.Series(100.0 + np.arange(60), index=idx)
        # remove the bar exactly 20 days after day 5
        gap = idx[5] + pd.Timedelta(days=20)
        close = close.drop(gap)
        label = compute_raw_forward_return_label(close, 20)
        assert np.isnan(label.loc[idx[5]])
        # neighbors unaffected
        assert label.loc[idx[6]] == pytest.approx((100.0 + 26) / (100.0 + 6) - 1.0)
...
class TestStoreAndPanel:
    ...
    def test_assemble_panel_labels_and_features(self, tmp_path: Path) -> None:
        ...
        d = sample["date"]
        expected = btc.loc[d + pd.Timedelta(days=20)] / btc.loc[d] - 1.0
