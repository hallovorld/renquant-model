...
record of the rejected design; this amendment replaces its schedule-selection
and evaluation sections. Phase 0 remains BLOCKED. No capital deployment or
...
they cannot be resolved by selecting the latest commit. A session with an
idiosyncratic arm failure is recorded and causes a terminal `NO-GO` unless the
pre-registered failure budget is met for a documented shared venue/calendar
outage. It is never silently dropped after observing returns.
...
any performance claim. Before the pilot, schedule, universe, artifact IDs,
cost model, terminal rule, MEE, planning effect, block rule, failure budget,
and terminal sample size rule are frozen.
...
value, the study is infeasible rather than tuned from pilot outcomes. Pilot
sizing uses a conservative upper confidence limit for long-run variance and a
block length fixed before pilot collection. It must show type-I no greater than
0.10 under `mu=MEE`, power at least 0.80 at the planning effect, and a fixed
terminal T within the declared maximum. Pilot means, signs, arm PnL, and
