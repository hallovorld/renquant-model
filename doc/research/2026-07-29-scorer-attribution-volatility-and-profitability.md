# Structural sensitivity of the live panel scorer: which features can move it
...
```
<umbrella>/.venv/bin/python tools/scorer_attribution_probe.py \
...
not answerable from the score DB — **and, as §5 now states, it is not fully
answerable from the artifact either.** What the artifact supports is a
*structural* question (which features CAN move this model, and by how much),
not an attribution of any particular historical score. The distinction is
load-bearing and an earlier revision of this note blurred it. What the
artifact does give:
...
## 5. AAPL: a consistent hypothesis, NOT an identified cause

Two facts, both measured:

1. **STD60 is among the model's largest structural levers** `[VERIFIED — probe
   B, §3]`.
2. **AAPL's STD60 fell 0.0639 → 0.0469 over the rally window**
   `[VERIFIED — computed from OHLCV this session]`.

Those are consistent with "it was marked down for rallying calmly." **They do
not establish it, and this note no longer claims they do.** An earlier revision
asserted that as the operative cause; withdrawn, for three reasons that the
method cannot get around:

- **The baselines are off-manifold.** Probe B draws each feature independently
  (`rng.normal(size=(n_baselines, n_features))`,
  `tools/scorer_attribution_probe.py:91`). Real feature vectors are strongly
  correlated — volatility measures co-move, profitability measures co-move — so
  the perturbation samples regions of feature space that never occur in data.
  That yields a valid *model-wide marginal sensitivity*, not the local
  behaviour at AAPL's actual position.
- **171 other features also moved** over that window, and their joint
  contribution is not accounted for. A one-feature-at-a-time sensitivity cannot
  be summed into an attribution when the inputs are dependent.
- **AAPL's archived per-day feature vectors do not exist**, so the
  counterfactual that would settle it — re-score AAPL's real 06-26 vector with
  only STD60 replaced by its 07-29 value — cannot be run.

What survives is worth keeping: the model's largest levers are volatility and
profitability, several momentum features are inert, and AAPL's realised
volatility did compress while it rallied. Turning that into a cause requires
retaining per-day feature vectors — which is the actionable follow-up.

The price-ratio mechanism of §1 is real in the features and is **also** not
established as the operative cause: those features carry gain but ~zero
marginal effect (§3). Naming it without probe B would have been a plausible,
well-evidenced, wrong answer — and naming STD60 *with* probe B is a better-
evidenced hypothesis, still not a proof.
...
- **Point 2 — CONFIRMED by measurement.** 38% of declared features are never
  split on; top-50 of 172 carry 68% of gain; two features carry most of the
