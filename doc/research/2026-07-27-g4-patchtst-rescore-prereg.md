...
| PHASE 2 (the remaining 40, resume) | same command with `--select-cutoffs <the full 43-list above verbatim>` — #81's resume partitions to the 40 absent folds (zero retrain/overwrite, fail-closed on missing/mismatched provenance, one auditable manifest with per-dispatch history) |
