...
|---|---|---|---|
| base | 0.046 | — | — |
| **TC 0.4 → 0.7** | 0.081 | **+75%** | engineering; the IR arithmetic is not a hypothesis, but treat the outcome as a **conditional scenario, not zero statistical risk** — TC, cost, participation, turnover, and realized P&L co-move when execution constraints change, so this needs a precommitted execution/P&L validation before it counts as free upside (review finding, 2026-07-25) |
...
## 7. STRUCTURAL DECOMPOSITION — two named-methodology tests (same day)

Frame: realized alpha = [skill + characteristic premia] × dispersion ×
capture (Grinold 1989; DGTW 1997). Each link measured on cached production
scores from an ad hoc scoring pass (`scores_real.parquet`/
`scores_placebo.parquet`) that was **not itself committed as a script** —
`structural_decomposition.py`'s docstring states the exact recipe to
reproduce those two inputs, but until someone runs it, §7.1-7.3 below are
**not independently reproducible from this repo alone** (model#69 review,
BLOCKER 1). Read the numbers below as a single-run measurement pending that
reproduction, not a certified/settled finding. A former TEST 3 (exit-stack
counterfactual) has been REMOVED from this repo — it read the umbrella's
`strategy_config.json` and production OHLCV directly, outside this repo's
GBDT-score/model-analysis boundary (model#69 review, BLOCKER 2); if pursued,
it belongs in the repo/harness that owns backtesting execution policy.

### 7.1 DGTW characteristic-matched benchmark — a candidate certified-skill signal, pending reproduction
...
- **The DGTW-adjusted spread measures at t=+2.92** in this single run —
  candidate evidence of within-cohort selection skill: *which high-vol name
  will run*, not just characteristic tilt. (Winsorized t=1.70 — the effect
  is tail-carried; the trimmed core is marginal.) Pending reproduction
  (see §7 header), this is NOT yet a certified finding.
...
VERDICTS NULL streak may be an artifact of the test statistic, not of the
hypotheses — **a hypothesis, not a recommendation**, pending reproduction of
§7.1 (see §7 header): if it survives, the gate-metric question (whole-
cross-section IC vs. characteristic-matched top-N spread) is worth its own
preregistered study before any gate changes.
...
### 7.4 Exit-stack counterfactual — REMOVED (repo boundary)

This section previously reported a production-stop-parameter counterfactual
(BULL_CALM stop/trailing params applied to real top-10 price paths). It has
been **removed** per model#69 review, BLOCKER 2: the producing script read
the umbrella's `backtesting/renquant_104/strategy_config.json` and
production OHLCV directly, which is backtesting/execution-policy analysis,
outside this repo's GBDT-score/model-analysis boundary — this is a
relocation, not a narrowing, since the code cannot stay here at all.

**Consequence for §6.3**: the removed measurement's directional finding
(stops mostly cost "whipsaw in the body," not tail amputation — i.e. §6.3's
"a tail-driven distribution is precisely the one that path-dependent stops
monetize worst" looked FALSIFIED) is no longer backed by anything
reproducible in this repo. Treat §6.3's original hypothesis as **OPEN** —
neither confirmed nor falsified — until a properly-scoped follow-up in the
repo/harness that owns backtesting execution policy reproduces (or fails
to reproduce) that counterfactual against a versioned, contract-level
input rather than a direct read of the umbrella's tree. No stop-policy
change (e.g. "σ-scaled stops") is authorized by this memo.

### 7.5 Caveats

Survivorship inflates all LEVELS (paired/blocked t-stats are the robust
part). DGTW cells are 27/date on ~280 names (~10/cell). §7.1-7.3 are not
yet independently reproducible from this repo (see §7 header) and §7.4 was
removed for repo-boundary reasons (see above) — none of §7's findings are
settled. Candidate skill is tail-dependent (winsorized t=1.70). One model
family, one panel.
