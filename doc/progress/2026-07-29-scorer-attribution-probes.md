# Progress: structural sensitivity of the live panel scorer
...
STATUS:   delivered. One committed probe + behavioural tests + a findings doc.
          ...
          RELOCATED from `renquant-orchestrator#615` per codex BLOCKER: the
          probe parses a production model artifact and implements
          model-attribution semantics, which are model-factory
          responsibilities, not daily orchestration.
          ALSO RESCOPED in the move, same review: the probe establishes
          model-wide STRUCTURAL sensitivity under a synthetic baseline, not
          attribution of any historical score. The claim that AAPL was
          "marked down for rallying calmly" is WITHDRAWN as an identified
          cause and restated as a hypothesis consistent with the evidence —
          see the research note's SS5 for the three reasons the method
          cannot settle it (off-manifold independent baselines, 171 other
          features moving jointly, and no archived per-day feature vectors
          to run the counterfactual on).

WHAT:     `tools/scorer_attribution_probe.py` — three read-only probes on
...
EVIDENCE: artifact:   `panel-ltr.alpha158_fund.json` (kind=panel_ltr_xgboost,
          ...
          command:    `<umbrella>/.venv/bin/python tools/scorer_attribution_probe.py
