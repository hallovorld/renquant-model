...
STATUS:   delivered (committed verifier + research memo). NOT a proposal; asks
          for no decision. Relocated from `renquant-orchestrator#601` per
          codex BLOCKER: this is model-evaluation research (corpus,
          evaluation method, reproducibility artifact), which
          `renquant-model` owns — orchestration should consume a published
          result, not host it. Content unchanged from orch#601's revision,
          which had already addressed 2 earlier MED findings (reviewable
...
EVIDENCE: artifact: `tools/breadth_precision_verify.py` (committed, this PR);
  ...
  scope:            `renquant-model` docs + one tool. No pin advanced, no
                    umbrella change, no live surface touched. GOAL-6 is an
                    orchestrator-side goal; this PR states the measurement
                    only — sequencing consequences belong in an
                    orchestrator-side proposal that cites this result, not
                    in this repo.
...
VERIFICATION:
          ...
          published `[VERIFIED — re-ran this session, output byte-identical
          to the memo's tables]`. `pytest tests/test_breadth_precision_verify.py`
          -> 8 passed (pin-abort contract, deterministic seeding, fit
          recovery, survivorship-probe logic; the substantive corpus/panel
          numbers are covered by the script re-run above, not by this
