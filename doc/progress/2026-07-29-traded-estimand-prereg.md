...
STATUS:   delivered (frozen prereg only). Contains NO result; the
          ...
          #96 is now in `main`, so this PR carries the same three files
          rebuilt directly on `main` — no stack, no rebase conflict, and §D
...
VERIFICATION:
          ...
          stated numbers exactly, run against post-merge `main` this
          session (renquant-model#96 landed `control_calibration`, so §D
          runs from the canonical import path with no PYTHONPATH override
          and no stack): fold means 1/30 = 3%, block means 2/30 = 7%,
          ~16% of valid experiments voided. The document contains no
